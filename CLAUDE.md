...
**Most recent ship — 2026-05-17 (evening) shipped D39 — Statcast reconciliation deep-dive.** Four sub-fixes closed Padres recon from 85% → 95%: D39a (spray) `hit_xy` is batter-relative (HR analysis across 1,889 MLB 2028 HRs); D39b game_type=0 filter on all L3 Statcast builders; D39c LA bucket recalibration (GB<12/LD<27/FB<52/PU≥52); D39d x-stats triple fix (integer-EV interpolation zero-bug, IE-style SUM/AB denominators, empirical scalers ×1.22 xBA / ×1.09 xSLG). xwOBA 0%→78%, xBA 0%→89-96%, xSLG 0%→89-97%, xERA 0%→87% (new). Plus save-aware report header. Commit `88664c6`.
