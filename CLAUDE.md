...
- `docs/DECISIONS.md` — append-only log of architectural/scope decisions with rationale (D1–D40).
...
**Current phase: Phase 4a — Audit Closure** (next, ~2-3 dev-days), then Phase 4b — Maximize the Warehouse (~5-6 dev-days), then Phase 5 — Baseball Almanac (~10-14 dev-days). **D40 (2026-05-17 evening) is the strategic commitment to the full phase arc**: 4a → 4b → 5 → 6 (multi-save) → 7 (AI analyst) → 8 (distribution, deferred). Goal: **richness × accuracy × flexibility, save-agnostic by construction**. The L0/L1/L2/L3 + L_REF + History layer architecture is sound and stays put; Phase 4 is additive — game-grain fact tables (`f_player_game_*`) for time-series, per-dump snapshots of derived stats (`f_*_history`), invariants watchdog (`_diamond_invariants`) self-validating against OOTP-authoritative `l0_team_*_stats` columns we ingest-but-never-use, per-player calibration (spray, xSLG) replacing flat scalers from D39. **See DECISIONS.md D40 for the full architectural commitment + per-phase plan + dependency chain + storage projection.**

**Most recent ship — 2026-05-10 shipped Phase 4a deliverable #1 — L0 inventory pass.** New `scripts/inventory_l0_coverage.py` (save-agnostic, runs via `--save NAME`; passes the source corpus through a word-boundary grep against every L0 column name) + `audit_output/l0_column_coverage.md`. Report has summary stats, top-orphan-tables ranking, category-bucketed Phase 4a wiring recommendations, and per-table breakdown. **Findings on The Fathers warehouse**: 69 L0 tables, 2,466 non-admin columns, **1,418 referenced (57%)**, **1,048 orphan (42%)**, **18 fully-consumed tables** (incl. PA log, awards, league leaders, IBS, salary history, streaks, team_record, team_roster, ...). Three tables D40 originally flagged as Phase 4a #2 wire candidates — `l0_players_league_leader`, `l0_players_individual_batting_stats`, `l0_players_salary_history` — are **already 100% consumed**; DECISIONS.md D40, PROJECT_STATUS.md, and BACKLOG.md updated with the refined wire-candidate list flowing into Phase 4a #2: `l0_team_pitching_stats` + variants (51 orphan cols — `ws`, `gbfbp`, `kbb`, `cgp`, `qsp`, ...), `l0_league_history_pitching_stats` (20) + `_fielding_stats` (17), `l0_players_value` (39 — per-position + per-side valuations), `l0_players_scouted_ratings` (45 — per-side splits), `l0_players_career_*_stats` (19 — cross-check vs `f_player_career`). All findings feed D40's invariants watchdog as candidate inputs.
