...
**Current phase: Phase 4b — Maximize the Warehouse** (next), then Phase 5 — Baseball Almanac, then Phase 6 (multi-save) → 7 (AI analyst) → 8 (distribution, deferred). **Phase 4a ✅ CLOSED 2026-05-10** — all 7 deliverables shipped (L0 inventory → L2 OOTP-cache wiring → MiLB 5-8 backfill → EV cutoffs → hit_loc decoding → multi-stint analysis → permanent-limitation writeup). The L0/L1/L2/L3 + L_REF + History layer architecture is sound and stays put; Phase 4b is additive — game-grain fact tables (`f_player_game_*`) for time-series, per-dump snapshots of derived stats (`f_*_history`), invariants watchdog (`_diamond_invariants`) self-validating against OOTP-authoritative columns wired in Phase 4a #2, per-player calibration (spray, xSLG) replacing flat scalers from D39. **See DECISIONS.md D40 for the full architectural commitment + per-phase plan + dependency chain + storage projection.**

**Most recent ship — 2026-05-10 shipped Phase 4a-extended — recon drive on never-re-examined gaps.** Post-Phase-4a-close, an honest audit surfaced 72 recon columns still under 100%. Five formula corrections in `reconcile.py` + `l3_advanced.py`: **(1) BIP filter `sac=0` removed** — IE counts sac flies AND sac bunts as BIP (probe: 59/78 exact vs 7/78 prior, cascade across 9+ downstream rate cols). **(2) BAR recalibration** EV≥99 LA[13..41] (27/74 exact vs prior 13/74). **(3) xwOBA scaler 1.03** in L3 builders (63/73 exact vs no-scaler 52/73). **(4) xERA formula refit** to `19.5*xwOBA - 2.5` post-scaler change (67/71 exact vs broken 30/71). **(5) LA bucket re-fit** to GB<11/LD 11-25/FB 26-50/PU≥51 (MAE 1.54 vs prior 2.54 — cascades into +20-45pp on GB/FB/HR-FB/IFFB across batting AND pitching). Net jumps: batting GB/FB 47→92, GB% 68→92, FB% 65→91, HR/FB 83→95, LA 53→94, HHi 68→94, xwOBA 78→92, BAR 19→40, BAR% 58→74; pitching GB/FB 61→92, GB% 76→95, FB% 73→95, xwOBA 82→96, xERA 87→97 (briefly regressed to 44 before refit, then jumped). Remaining gaps documented as either structural (Pull/Cent/Oppo at 38-54% — D39+Phase-4a-ext both confirm 1D hit_xy limit), data-scope (BIP 80-82% — 40 DSL/foreign players have summary stats but no PA event log), or sparse-denominator noise (BUH%, BAR ±1 count per player). DATA_NOTES.md "Phase 4a-extended recon drive" section captures the full scorecard + root causes.
