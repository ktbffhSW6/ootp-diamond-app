...
- `docs/DECISIONS.md` — append-only log of architectural/scope decisions with rationale (D1–D26).
...
L_REF (planned, D26) reference data shared across saves, ingested from
                  `<docs>/Out of the Park Developments/OOTP Baseball 27/`:
                    ...
                    era_stats / era_stats_minors (league averages per era)
                    Master / MiLBMaster (OOTP↔Lahman crosswalk)
                    db_structure_complete_ootp21_* (canonical schema docs)
...
- **OOTP parent-folder reference data (D26 finding)** — `<docs>/Out of the Park Developments/OOTP Baseball 27/` contains ~500MB of static reference data that doesn't change per save: `database/pt_ballparks.txt` + `era_ballparks.txt` (authoritative park dimensions + factors, beats our hand-coded `web/lib/stadiums.ts` and Lahman BPF), `database/era_stats.txt` (league averages per era), `stats/Master.csv` (OOTP↔Lahman crosswalk including BBref + Retrosheet IDs — replaces our Chadwick lookup), `logos/` (1,829 PNGs disguised as `.oi`), `ballcaps/` (343), `database/db_structure_complete_ootp21_*.txt` (canonical schema docs we'd been reverse-engineering). v2 architecture introduces an **`L_REF` ingest layer** sitting alongside L0-L3, ingesting from this parent folder once and joining into existing tables. Treat the parent folder as **read-only canon**; never write into it.
