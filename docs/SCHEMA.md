...
## TL;DR — the five layers (eight counting `L_REF`, `L_IE`, plus the Phase 4b `L2_*` extensions; nine if `L_NEWS` is ever pulled in per D28)
...
| **L2_OOTP** *(Phase 4a #2, [D40](DECISIONS.md#d40))* | OOTP-cache passthrough fact tables — `f_team_season_{batting,pitching,fielding}_ootp`, `f_player_stint_{batting,pitching,fielding}_ootp`, `f_league_season_{pitching,fielding}_ootp`, `f_player_value_current`, `v_player_ratings_by_side`. Surface the previously-orphan OOTP-cached rate stats as named cols. Each builder asserts orphan column names via `_assert_columns_present`. Feeds the D40 invariants watchdog. | L1 + L0 | After L2 | Full rebuild — DROP/CREATE |
| **L2_game_grain** *(Phase 4b Tier A 2026-05-14, [D40](DECISIONS.md#d40))* | Per-(player, year, game) fact tables: `f_player_game_batting` (1.1M rows Padres / 430K Sox smoke; 18 stat cols), `f_player_game_pitching` (355K / 134K). Cross-dump dedup via ROW_NUMBER ORDER BY dump_date DESC; JOIN to deduped `l0_games` for canonical date. `ORDER BY (player_id, date)` in CTAS — DuckDB physically sorts on disk. Game-grain fielding deferred (no L0 source). Unblocks rolling windows + Phase 5 Almanac. | L0 directly | After L2_OOTP | Full rebuild — DROP/CREATE |
| **L2_history** *(Phase 4b Tier B 2026-05-14, [D40](DECISIONS.md#d40))* | Per-dump SCD2-style history snapshots sourced from L0 (L1 events collapse to MAX(dump_date)). `f_player_season_batting_history` (15M rows Padres), `f_player_season_pitching_history` (8M), `f_player_career_history` (3M, career rollups per dump). Cross-stint dedup at (player, year, team, league, level, split, stint, dump_date) grain. Powers `/api/players/{id}/trajectory` and future sparkline UI. | L0 directly | After L2_game_grain | Full rebuild — DROP/CREATE |
| **L3 — derived** | Sabermetric stats, league constants, park factors, `player_movements`, awards rollups, x-stats with POW-aware calibration | L2 + L1 + L_REF | After L2 | Full rebuild — DROP/CREATE |
...
| **`_diamond_invariants`** *(Phase 4b D40 watchdog 2026-05-14)* | Per-(dump_date, scope_type, scope_id, year, level_id, metric) drift comparator. dump_value (from L2_OOTP) vs derived_value (from L2 season facts), delta + tolerance + status (green/amber/red). 9 invariants active (team AVG/OBP/SLG/ERA/WHIP/K9/BB9 + PA/HR event counts). 100.0% green on Padres post the career-event scope fix (D43). | L2_OOTP + L2 | After L_IE | Append-per-dump, deletes-and-rewrites the current dump's rows |
| **L_REF — reference** *(Slices 1-5 + 6 data layer shipped 2026-05-14, [D26](DECISIONS.md#d26--l_ref-reference-layer-from-ootp-parent-folder-data) + [D27](DECISIONS.md#d27--l_ref-is-per-save-frozen-at-first-ingest-opt-in-refresh) + [D29](DECISIONS.md#d29--l_ref-rollout-analytical-slices-shipped-cosmetic-deferred-2026-05-14))* | OOTP-canonical analytical lookup tables (xwOBA / xBA / xSLG / xiso / RE288 / WPA / LI / pi grids), historical baselines (era_stats / era_stats_minors / era_modifiers / era_fielding / total_modifiers), park data (pt_ballparks + era_ballparks with handedness splits), crosswalks (Master.csv 24,746 rows / MiLBMaster 212k rows), seed pool, weather, financials, plus future cosmetic (HoF plaques). **27 tables / 575,587 rows** | OOTP parent folder `<docs>/Out of the Park Developments/OOTP Baseball 27/` (`misc/*.txt`, `database/*.txt`, `stats/*.csv`) | **First `diamond ingest` only** — frozen for save lifetime per D27; opt-in refresh via `diamond ingest --refresh-lref`. Implementation: `src/diamond/schema/l_ref.py` with `ensure_lref()` / `compute_lref_diff()` / `refresh_lref()`; provenance in `_diamond_settings.lref.{frozen_at,source_root,ootp_version,table_count,files_json}` | Read-only canon — Diamond never writes to the parent folder. Per-save snapshot mirrors OOTP's own engine convention. **Active consumers (D29)**: `lref_xwoba/xba/xslg_table` → `_xwoba/xba/xslg_lookup` views → `_f_pa_event_xstats` → `f_player_season_xstats_batting/pitching` (Slice 2); `lref_era_stats` + `lref_era_stats_minors` → `_lg_constants_advanced_imported` view (Slice 4 + 5); `lref_era_ballparks` → `_park_factor_resolved` view (Slice 3); `lref_pt_ballparks` → `/api/parks` (Slice 6). Inactive (loaded but unused): xiso/re288/li/wpa/pi tables, era_modifiers/fielding/total_modifiers, financials, weather, default_players, master, milb_master, teams_history, milb_leagues/teams, eos/od_rosters, uni_numbers, series_post |
| **L_IE — IE display values** *(shipped 2026-05-14, [D42](DECISIONS.md#d42))* | Per-save OOTP-IE-canonical display values for org-roster players. 21 `lie_*` tables (~5,649 rows on Padres) ingested from `<save>/import_export/*_organization_-_roster_*.csv`. Three unified views parse OOTP display strings to typed numerics: `v_lie_player_batting_display` (38 cols), `v_lie_player_pitching_display` (26), `v_lie_player_fielding_display` (19). API CTEs LEFT JOIN + COALESCE for bit-for-bit OOTP IE on single-stint org-roster latest year. | Save folder `<save>/import_export/` | DROP-and-rebuild on every warehouse refresh (point-in-time snapshot, unlike L_REF freeze). User must manually re-export via OOTP "Reports → Roster Exports" after each sim advance. | Per-save independent. Best-effort + skip-on-missing — saves without IE exports get empty stub views; downstream LEFT JOINs return NULL and fall through to derivations. |
