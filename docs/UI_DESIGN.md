...
16. ✅ **Situational splits** on player page — done 2026-05-12. Situational batting + pitching sections, multi-year coverage, **14 splits per (year, level)** organized into five clusters: **Leverage** (All / RISP / RISP 2-out / Late & Close) · **Bases** (empty / loaded) · **Platoon** (vs L / vs R, side-aware labels) · **Counts** (first pitch / two strikes / full count) · **Spray** (pull / center / oppo, BIP-only). Slash + counting per row. OPS color-coded vs the All baseline (emerald ≥25 pts in player's favor, rose ≥25 pts against; inverts for pitchers). Spray splits skip color coding — denominator semantics differ. Switch-hitters resolve to opposite of pitcher's throwing hand. Empirically verified along the way that `hit_xy` is **batter-relative** (mean hit_xy on HRs ≈71 for both LHB and RHB — same pull-side band for both hands), corrected DATA_NOTES. Backed by `_fetch_situational(con, player_id, side)` reading from the multi-year `f_pa_event` + LEFT JOINs to `players_current` for handedness. Verified: Devers 2029 spray Pull 12 HR / Center 15 HR / Oppo 0 HR (12+15+0=27 total), Two strikes .585 → Full count .820; Crochet 2029 vs LHB .561 OPS allowed (dominance) vs RHB .697; switch hitter Leo De Vries 2029 routes 184 vs LHP / 513 vs RHP cleanly.
...
18. ✅ **Compare under Explore** — done 2026-05-12. `/explore/compare?ids=` lives. `GET /api/compare?ids=` returns slim cards (career batting + pitching lines + WAR sparkline series + headline metric) for ≤4 players side by side. Cross-era via D20 baselines (Bonds 2001 / Trout 2018 / Skubal 2029 all carry full advanced numbers). Empty state surfaces three demo deep-links. Chart-stack decision deferred — Sparkline component is the v1 backstop; Vega-Lite vs Plotly lands when spray charts / EV-LA / distributions need it.
...
23. **League view content** — standings (item 15) shipped; leaderboards, awards races, free-agent pool still pending.
24. **World view content** — all-leagues browser; thin until scope expands.
25. **Universes + chart builder + scatter** *(bundled)* — Vega-Lite + Plotly. Lives under Explore. *Partial:* per-player **spray** + **EV-LA scatter** shipped 2026-05-13 (D23 — Observable Plot picked for cohort viz). Chart-builder JSON-spec authoring still deferred until the slice that demands it.
