...
16. ✅ **Clutch / RISP / bases / platoon splits** on player page — done 2026-05-12. Situational batting + pitching sections, multi-year coverage, **8 splits per (year, level)**: All / RISP / RISP 2-out / Late & Close / Bases empty / Bases loaded / vs LHP-or-LHB / vs RHP-or-RHB. Slash + counting per row. OPS color-coded vs the All baseline — emerald when ≥25 pts in the player's favor, rose when ≥25 pts against. Color logic inverts for pitchers. Side-aware labels for the platoon splits. Switch-hitters resolve to the opposite of the pitcher's throwing hand for the pitcher view. Backed by `_fetch_situational(con, player_id, side)` reading from the now-multi-year `f_pa_event` + LEFT JOINs to `players_current` for handedness. Verified end-to-end: sanity invariants hold (`vs_left + vs_right = all`); Devers 2029 vs RHP .788 vs LHP .746 (textbook LH bat), bases loaded 1.418 OPS in 15 PA; Crochet 2029 vs LHB .561 OPS allowed (LHP dominance), vs RHB .697 (platoon penalty); switch hitter Leo De Vries correctly routes 184 PA vs LHP / 513 PA vs RHP.
