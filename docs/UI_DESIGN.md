...
16. ✅ **Clutch / RISP splits** on player page — done 2026-05-12. New "Situational batting" section. Four splits per (year, level): All / RISP / RISP 2-out / Late & Close. Slash + counting per row. OPS in a split row colors emerald (≥25 pts above All baseline = clutch) or rose (≤-25 = choke); smaller gaps stay neutral (single-season-cut noise). Backed by `_fetch_situational_batting` joining `f_pa_event` → `games_event` for the regular-season filter. v1 inherently single-season because OOTP replaces the per-PA log each rollover. Verified: Devers 2029 RISP .881 (emerald) / 2-out RISP .689 (rose) / Late & Close .685 (rose). Pitcher splits deferred.
