...
| 2 | **Wire authoritative team-stat columns + valuation cache** — concrete targets from #1's inventory: (a) `l0_team_pitching_stats` + `_history_pitching_stats` + bullpen/starting variants — 51 orphan cols incl. `ws`, `gbfbp`, `kbb`, `cgp`, `qsp`, `winp`, `svp`, `bsvp`, `gfp`, `pig`, `r9`, `h9`, `sa`, `da`, `ta`, `ra`; (b) `l0_league_history_pitching_stats` (20) + `_fielding_stats` (17) — league-level OOTP-cached aggregates; (c) `l0_players_value` (39 orphans, 86%) — per-position (`overall_sp/rp/c/1b/2b/3b/ss/lf/cf/rf`) + per-side (`*_value_vsl/vsr`) valuations; (d) `l0_players_scouted_ratings` (45 orphan side-splits); (e) `l0_players_career_*_stats` (19 orphans across 3 tables) — career rollups OOTP pre-computes, cross-check vs our `f_player_career`. Feeds **D40 invariants watchdog** in Phase 4b — every wired column becomes a possible invariant input | 1-2 hrs | Unused-authoritative-data class of bugs |
