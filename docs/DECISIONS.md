...
|---|---|---|---|
| 1 | **L0 inventory pass** ✅ **DONE 2026-05-10** — `scripts/inventory_l0_coverage.py` enumerates every column in every L0 table + word-boundary-greps references across `src/diamond/**`, `web/{app,components,lib}/**`, and `scripts/**`; outputs `audit_output/l0_column_coverage.md` with summary stats, top-orphan-tables ranking, category-bucketed wiring recommendations, and per-table breakdown. **Findings**: 2,466 non-admin L0 columns; **1,418 referenced (57%)**, **1,048 orphan (42%)**; 18 fully-consumed tables. Stale D40 callouts: `players_league_leader`, `players_individual_batting_stats`, `players_salary_history` are **already 100% consumed** — not wire candidates. True remaining wire candidates flow into #2 below | 1 hr | Definitive answer to "what did we miss at the start" |
| 2 | **Wire authoritative team-stat columns + valuation cache** ✅ **DONE 2026-05-10** — new `src/diamond/schema/l2_ootp.py` adds 9 fact tables + 1 view passing OOTP-cached aggregates through to L2: `f_team_season_{batting,pitching,fielding}_ootp` (PK team_id/year), `f_player_stint_{batting,pitching,fielding}_ootp` (per-stint passthrough preserving OOTP-cached rate cols), `f_league_season_{pitching,fielding}_ootp`, `f_player_value_current` (latest-dump per-player valuations — 39 orphans surfaced), `v_player_ratings_by_side` (per-side scouted-rating splits — 45 orphans surfaced). Each builder calls `_assert_columns_present` against an explicit `_*_ORPHAN_COLS` tuple so an OOTP version-bump that drops/renames a cache column fails the warehouse build loudly instead of silently dropping the watchdog input. Wired into `rebuild_l1_l2` orchestration after L2, before L3. **Outcome**: 1,418 → 1,681 referenced cols (57% → 68%); **263 orphans closed in 10 target tables, all now at 100% coverage**: `l0_team_history_{batting,pitching,fielding}_stats(_stats)` (16+1+2), `l0_team_{pitching,batting,fielding,bullpen,starting}_stats` (all 5 derived-from-shared-names), `l0_players_career_{batting,pitching,fielding}_stats` (1+4+14), `l0_players_value` (39), `l0_league_history_{pitching,fielding}_stats` (20+17), `l0_players_scouted_ratings` (45). All `_assert_columns_present` checks pass + `make smoke` clean | 1 hr | Unused-authoritative-data class of bugs |
| 3 | **MiLB levels 5-8 advanced-stats backfill** ✅ **DONE 2026-05-10** — Phase 4a #1's inventory exposed that the imported league-constants view (`_lg_constants_advanced_imported`) used `MIN(level_id)` to crosswalk `lref_era_stats_minors` rows. **Bug A (level fan-out)**: OOTP's 2021 MiLB reorg means leagues 209-213 + 252 (Northwest/SAL/Midwest/Cal/Carolina/FSL) appear at *both* L4 (modern) and L6 (historical) in a single save — the MIN-join left every L6 row orphan. Fixed by switching to `milb_levels_per_league` (PLURAL — one row per (league_id, level_id) seen in `f_player_season_batting`). **Bug B (crosswalk gap)**: added `(237, 'American Association')` (covers 1903-1997) + `(253, 'Pioneer League')` (1939-2024) to `milb_xwalk`. **Result**: L6 NULL pct 100% → 85.3% (~1,000 closed), L7 100% → 84.3% (~140 closed). **Remaining ~7,500 nulls at L5/L6/L7/L8 are permanent** — foreign professional leagues (KBO, DSL, Korean Futures), MLB-affiliated rookie complex leagues (AZ/FL Complex), modern post-1998 independents (Atlantic, Frontier), and unknown-name OOTP-internal placeholder leagues. Documented in DATA_NOTES.md "MiLB levels 5-8 advanced-stats coverage" section | 30 min | Pre-2026 minor-league rows rendering "—" everywhere |
| 4 | **EV-bucket OOTP-canonical calibration** ✅ **DONE 2026-05-10** — grid-searched (soft, solid) cutoff pairs against the Padres IE corpus: 74 batters × 12,506 BIP events with IE-reported Soft/Avg/Solid%, search space soft ∈ [70..80] × solid ∈ [90..96]. Best pair **(76, 95)** at MAE=1.68pp vs prior (75, 95) at 1.92pp. Pitching corpus (73 pitchers × 12,780 BIP allowed) independently agreed: MAE 1.81 vs 2.06pp. The **95-mph solid threshold matches MLB-Statcast's hard-hit convention exactly**; the 76 soft floor is 1 mph above prior 75 — marginal but consistent improvement. Applied to `BATTING_SUPERSTATS_CTE` + `PITCHING_SUPERSTATS_CTE` in `reconcile.py`. **Recon improvements**: batting Soft% 58→68 (+10pp), Avg% 58→60 (+2pp), pitching Soft% 53→74 (+21pp), Med% 64→73 (+9pp); Solid% unchanged (cutoff already correct). DATA_NOTES "OOTP-canonical EV cutoffs" section documents the calibration | 30 min | DATA_NOTES "OOTP EV cutoffs unknown" |
| 5 | **`hit_loc` semantic decoding** ✅ **DONE 2026-05-10** — grid-searched hit_loc infield/outfield boundary against IE-reported `IFH%` across Padres corpus, testing three FanGraphs formula variants. Best fit: **F2 (IFH / all-GB, FanGraphs canonical)** at cutoff **22** with MAE=3.54pp — beats F1 (IFH / GB-singles, MAE 5.62) and F3 (IFH / GB-hits, MAE 5.09). **Decoding**: `hit_loc ≤ 22 = infield zone`, `hit_loc ≥ 23 = outfield`. Natural break at 22/23 is sharp (11 GB-hits at 22, 133 at 23 — 12× jump). Applied to `BATTING_SUPERSTATS_CTE`: `IFH = COUNT GB-singles where hit_loc ≤ 22`, `IFH% = 100*IFH/total-GB`. **Recon result**: IFH% from NULL → **60% match within ±4pp**. Higher precision would need multi-dim decoding (hit_loc × EV × LA), deferred to Phase 4b/5. DATA_NOTES "hit_loc semantic decoding" section captures the calibration | 30 min | `IFH%` permanently NULL, sharper-spray blocker |
| 6 | **Multi-level OPS+/ERA+ formula refinement** ✅ **DONE 2026-05-10** — investigation finding: **original hypothesis was wrong**. The "~5-10pp error" was not a level-weighted park-factor bug. Full Padres-corpus sweep (28 multi-stint players in IE) showed: single-stint median \|OPS+ - IE\| = 2; multi-stint median PA-weighted gap = 2; P90 ≈ 5; mean = 5.4 (pulled by 2 outliers); max = 88. Per **D11**, rate stats are intentionally never rolled up across levels — `f_player_season_advanced_*` produces one row per (player, year, league, level). PA-weighted aggregation of our per-level rows reproduces IE's combined display value within ±2 OPS+ for the typical case. **Outlier gaps (>20 OPS+)** come from players with **foreign / winter-league stints** (e.g. L11 Caribbean Winter, KBO) that IE's org-roster display excludes — not a formula bug. **No formula change**; corrected understanding in DATA_NOTES "Cross-level player caveat" + "Multi-stint OPS+/ERA+ presentation gap" sections. D11 architecture is preserved as correct | 1 hr | BACKLOG carry-forward item #1 |
