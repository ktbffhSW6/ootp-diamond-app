...
**Last updated**: 2026-05-14 (mid-day) — **Phase 3: L_REF Slices 1 + 2 shipped.** Slice 2 (calculation-parity swap) — two new fact tables `f_player_season_xstats_batting` (20,787 rows) + `_pitching` (21,504 rows) materialize OOTP-canonical xwOBA / xBA / xSLG per BIP via bilinear interpolation of `lref_xwoba_table` / `lref_xba_table` / `lref_xslg_table`. Implementation: `_xwoba_lookup` / `_xba_lookup` / `_xslg_lookup` long-form views via DuckDB `UNPIVOT`; `_f_pa_event_xstats` per-BIP interpolation view (1D linear along EV, LA integer in OOTP at-bat dump); season aggregation tables keyed on `batter_id` / `pitcher_id`. Wired into `PlayerAdvancedBattingRow` + `PlayerAdvancedPitchingRow` schemas with NULL-safe LEFT JOIN; player page Advanced columns show `wOBA | xwOBA` side-by-side. `xwOBA` / `xBA` / `xSLG` added to leaderboards stat catalog (2029 MLB xwOBA leaders: Henderson .315, Okamoto .309, Nimmala .302). Glossary entries with KaTeX formulas. Devers 2026-2029 actual wOBA .320-.355 vs xwOBA-BIP .264-.288 (gap = K/BB/HBP exclusion, expected); Crochet allowed-xwOBA .277 → .266 → .236 → .237 (pairs with FIP trajectory). Deferred from Slice 2: xISO (needs LSA classifier from (LA, EV) — OOTP at-bat dump doesn't carry LSA); RE24/WPA/LI from re288/wpa/li_table. Slice 3 (era-aware park factors with LH/RH splits via `lref_era_ballparks`) is next-up.
...
**Next — L_REF Slice 3 (era-aware park factors, D22 v2)** is the priority next-up. Slice 1 + 2 shipped 2026-05-14; Slice 3 swaps `_park_factor_resolved` to read from `lref_era_ballparks` (3,105 historical park-seasons with LH/RH splits) instead of Lahman BPF/PPF, and applies handedness-aware blending to OPS+/wRC+ via player.bats (switch-hitters get a 60/40 blend per Tango).
...
| 2 | ✅ **Calculation-parity swap** (shipped 2026-05-14) | `f_player_season_xstats_batting` (20,787) + `_pitching` (21,504) via bilinear interpolation of `lref_xwoba_table` / `lref_xba_table` / `lref_xslg_table`; player page wOBA \| xwOBA side-by-side; leaderboard catalog + 3 glossary entries. xISO + RE24/WPA/LI deferred (need LSA classifier or larger scope) |
| 3 | **Era-aware park factors** ← *next* | Swap `_park_factor_resolved` to read from `lref_era_ballparks` (LH/RH splits); blend per `players.bats` for switch-hitters |
