...
L_IE display routing Slice 1              ✓ 2026-05-14 (advanced bat/pit; bit-for-bit OOTP IE)
─────────────────────────────────────────
...
| 2 | **Authoritative team-stat + valuation cache wiring** ✅ **DONE 2026-05-10** — `src/diamond/schema/l2_ootp.py` adds 9 new L2 facts + 1 view: `f_team_season_{batting,pitching,fielding}_ootp` (team-year), `f_player_stint_{batting,pitching,fielding}_ootp` (per-stint), `f_league_season_{pitching,fielding}_ootp`, `f_player_value_current`, `v_player_ratings_by_side`. Each builder asserts expected orphan column names via `_assert_columns_present`. Outcome: 1,418 → 1,681 cols referenced (57% → 68%); **263 orphans closed**; 10 target tables now at 100% coverage. Feeds D40 invariants watchdog in Phase 4b — every wired column is a candidate invariant input | Unused-authoritative-data class |
| 3 | **MiLB levels 5-8 advanced-stats backfill** ✅ **DONE 2026-05-10** — `l3_advanced.py` level fan-out fix (Bug A: `MIN(level_id)` → `milb_levels_per_league` PLURAL) + crosswalk extension (Bug B: added American Association + Pioneer League). L6 NULL 100% → 85% (~1,000 closed); L7 100% → 84% (~140 closed). Remaining nulls = foreign/complex/independent/unknown leagues with zero `lref_era_stats_minors` data; documented as permanent | Pre-2026 minor rows rendering "—" |
| 4 | **EV-bucket OOTP-canonical calibration** ✅ **DONE 2026-05-10** — grid-search across Padres corpus (74 batters × 12,506 BIP + 73 pitchers × 12,780 BIP) chose **(76, 95)** at MAE 1.68pp / 1.81pp vs prior (75, 95) at 1.92 / 2.06pp. Solid threshold matches MLB-Statcast convention exactly. Applied to `reconcile.py`; recon Soft% +10-21pp, Med% +2-9pp | DATA_NOTES "OOTP EV cutoffs unknown" |
| 5 | **`hit_loc` semantic decoding** ✅ **DONE 2026-05-10** — grid-search against Padres IFH% with three FanGraphs formula variants. **Decoding**: hit_loc ≤ 22 = infield, ≥ 23 = outfield. F2 (IFH / all-GB) at cutoff 22 wins at MAE=3.54pp. IFH% recon NULL → 60% match | `IFH%` permanently NULL, sharper-spray blocker |
| 6 | **Multi-level OPS+/ERA+ refinement** ✅ **DONE 2026-05-10** — **hypothesis was wrong**. PA-weighted aggregation of our per-level rows reproduces IE's combined value within ±2 OPS+ for median multi-stint player. Outliers come from foreign / winter-league stints IE excludes — not a formula bug. D11 architecture preserved; DATA_NOTES corrected | BACKLOG carry-forward #1 |
| 7 | **Permanent-limitation writeup in DATA_NOTES.md** ✅ **DONE 2026-05-17 evening** — codes 44+49, OOTP developed-pitch state, F-tier pitch-tracking, players_pitching.csv scouting-mode zeros | Audit hygiene |

**Phase 4a ✅ CLOSED 2026-05-10.** All 7 deliverables shipped. `Audit phase — carry-forward` section: 100% resolved or marked permanent. Zero ambiguity.
